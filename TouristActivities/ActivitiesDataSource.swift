...
//  TouristActivities
//
//  Created by Wiwat Thaphon & Mahin Pathan on 2023-06-15.
//
...
class ActivitiesDataSource : ObservableObject{
    ...
    func removeFavoriteByOffset(offsets:IndexSet){
        var favoriteActivitiesId = self.getUserFavoritesActivityIds()
        favoriteActivitiesId.remove(atOffsets: offsets)
        UserDefaults.standard.set(favoriteActivitiesId, forKey: self.getLoggedInUserFavoriteKey())
        self.setFavoritesList()
    }
    
    
    func removeFavorite(activityId:String){
        var favoriteActivitiesId = self.getUserFavoritesActivityIds()
        
        if let index = favoriteActivitiesId.firstIndex(of: activityId) {
            favoriteActivitiesId.remove(at: index)
        }
        UserDefaults.standard.set(favoriteActivitiesId, forKey: self.getLoggedInUserFavoriteKey())
        self.setFavoritesList()
    }
    ...
    func getUserFavorites() -> [Activity]{
        ...
        for index in 0...activitiesList.count-1 {
            if activitiyIds.contains(activitiesList[index].id ){
    ...
    func isFavorite(activity:Activity)->Bool{
        let favoriteIds = self.getUserFavoritesActivityIds()
        return favoriteIds.contains(activity.id)
    }
}
